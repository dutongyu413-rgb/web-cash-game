...
function validateEffect(effect, location) {
  ...
  if (effect.type === "schedule_random_savings_effect") {
    ...
    effect.outcomes.forEach((outcome, index) => {
      ...
      assert.ok(
        hasFixedAmount || hasIncomePercent || hasIncomeLossPercent || hasSavingsCost,
...
test("事件卡 ID 唯一且基础字段完整", () => {
  assert.equal(eventCards.length, 71, "事件卡数量发生变化时，请同步审查卡池和测试基线");
  const ids = new Set();
...
test("十三张职业事件分别绑定一个有效身份且不会混入其他职业", () => {
  ...
  assert.equal(careerCards.length, 13);
